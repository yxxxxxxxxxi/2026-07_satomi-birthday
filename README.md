...
My Maps では可能です。ただし、この CSV 出力から自動で色指定するより、My Maps 側でスタイル調整する方が実務上扱いやすいです。

1. 左サイドバーで対象レイヤーまたは個別ピンを選ぶ
2. スタイル変更のアイコンを開く
3. 色を赤、オレンジ、青などに変更する
4. 必要ならアイコン形状も変更する
...
### My Maps を HTML に埋め込む方法

可能です。My Maps の `自分のサイトに埋め込む` で得られる `iframe` URL を HTML に入れます。

このプロジェクトでは `config/trip.yaml` の `mymaps_embed_url` と `mymaps_view_url` を使って、ダッシュボードに `地図` タブを表示できます。

今回の埋め込み URL:

```text
https://www.google.com/maps/d/embed?mid=1u1mFZ5mAd8fxwiH_t1S5K0g37RC7WN0
```

注意点:

- 地図の共有設定が閉じていると iframe 表示できないことがあります
- 理実に見せるなら、地図側も閲覧可能な共有設定にしておく必要があります
- 埋め込みは閲覧向きで、細かい編集は My Maps 本体で行う方が扱いやすいです

## HTML ダッシュボードの確認方法

- `python3 scripts/build_dashboard.py` で `output/travel_dashboard.html` を生成します
- ブラウザで開くと、概要、地図、旅程表、観光候補、宿泊候補、食事候補、移動、予算、予約・ToDo、誕生日演出、メモのタブを確認できます

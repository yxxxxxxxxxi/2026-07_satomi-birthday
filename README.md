...
GitHub Pages 用の公開ファイルとして `output/index.html` も同時に生成されます。
...
このプロジェクトには [.github/workflows/deploy-pages.yml](/Users/yuki/development/travel-planning/2026-07_satomi-birthday/.github/workflows/deploy-pages.yml) を含めています。`main` ブランチへ push されると GitHub Actions が `python scripts/build_all.py` を実行し、`output/` を GitHub Pages にデプロイします。

### 公開の流れ

1. GitHub にリポジトリを作成する
2. `main` に push する
3. リポジトリ Settings の Pages で `Source: GitHub Actions` を選ぶ
4. Actions のデプロイ完了後、`https://<owner>.github.io/<repository>/` で閲覧する
